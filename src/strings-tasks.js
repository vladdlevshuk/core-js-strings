...
function removeLastOccurrences(str, value) {
  ...
  if (lastIndex !== -1) {
    ...
    const after = str.substring(lastIndex + value.length);
    return before + after;
  }
...
function reverseString(str) {
  ...
  return arrayOfSymbols.reverse().join('');
}
...
function orderAlphabetically(str) {
  ...
  return arrayOfSymbols.sort().join('');
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (arrayOfVowels.includes(str[i])) countOfVowels += 1;
  }
...
function reverseWords(str) {
  ...
  for (let i = 0; i < arrayOfWords.length; i += 1) {
    const reversedWord = arrayOfWords[i].split('').reverse().join('');
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (currentChar === currentChar.toLowerCase()) {
      invertedStr += currentChar.toUpperCase();
    } else {
      invertedStr += currentChar.toLowerCase();
    }
  }
...
function encodeToRot13(str) {
  const standart = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const rot13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';

  const encodedStr = str.replace(/[a-zA-Z]/g, (char) => {
    const index = standart.indexOf(char);
    return rot13[index];
  });
